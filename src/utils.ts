...
export function removeOutliersWithSpread(
  ...
  const sellRateStdDev = calculateStandardDeviation(
    sellRateValues,
    sellRateMean
  );
...
export function calculateSpread(baseRate: number): {
  ...
  const buyRate = baseRate * (1 - halfSpread) * 1.001;
  const sellRate = baseRate * (1 + halfSpread) * 1.02;
...
export function calculateConfidenceScore(
  ...
  return Number(Math.min(100, (1 - standardDeviation / mean) * 100).toFixed(2));
}
